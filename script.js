...
// State & Session Management - Using v2 to force clear any previous cached data
const currentPath = window.location.pathname;
const isHomePage = currentPath.endsWith('index.html') || currentPath.endsWith('/') || currentPath === '' || currentPath.includes('index.html');

// Reset logic: if no active session, redirect to home and clear cart
if (!sessionStorage.getItem('novatech_session_v2')) {
    if (!isHomePage) {
        window.location.href = 'index.html';
    } else {
        sessionStorage.setItem('novatech_session_v2', 'true');
        sessionStorage.setItem('novatech_cart_v2', '[]'); // Explicitly empty
    }
}

// Load cart only if session is active
let cart = [];
const savedCart = sessionStorage.getItem('novatech_cart_v2');
if (savedCart && sessionStorage.getItem('novatech_session_v2')) {
...
const addToCart = (productId, title, color = null, capacity = null, priceOverride = null) => {
    ...
    sessionStorage.setItem('novatech_cart_v2', JSON.stringify(cart));
    updateCartCount();
